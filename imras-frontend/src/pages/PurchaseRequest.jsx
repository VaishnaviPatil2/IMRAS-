...
const PurchaseRequest = () => {
  ...
  const canCreatePR = isManager; // ✅ ONLY Manager creates PRs (not Admin)
  const canEditPR = isManager; // ✅ ONLY Manager edits PRs (not Admin)
  const canDeletePR = isManager; // ✅ ONLY Manager deletes PRs (not Admin)
  const canConvertToPO = isManager; // ✅ ONLY Manager converts PR to PO (not Admin)
  const isReadOnly = isWarehouse || isAdmin; // ✅ Warehouse + Admin are read-only
  const hasOverrideAccess = isAdmin; // ✅ Admin has emergency override only

  // Load data on component mount
  useEffect(() => {
    console.log('🚀 PR Component mounted, user:', user);
    ...
    // Listen for automatic PR creation events
    const handleAutomaticPRsCreated = (event) => {
      console.log('🔄 Automatic PRs created, refreshing data...', event.detail);
      loadPurchaseRequests();
    };
  ...
  const loadPurchaseRequests = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Debug: Check for automatic PRs
        const autoPRs = response.data?.filter(pr => pr.isAutoGenerated) || [];
        console.log('🤖 Automatic PRs found:', autoPRs.length);
        autoPRs.forEach(pr => {
          console.log(`   - ${pr.prNumber}: ${pr.item?.name} (${pr.urgencyLevel})`);
        });
      } else {
  ...
  const loadItems = async () => {
    try {
      ...
      if (response.data) {
        ...
        setItems(items);
        console.log('✅ Items loaded (direct):', items.length);
      }
    } catch (err) {
  ...
  const loadWarehouses = async () => {
    try {
      ...
      if (response.data) {
        ...
        setWarehouses(warehouses);
        console.log('✅ Warehouses loaded (direct):', warehouses.length);
      }
    } catch (err) {
  ...
  const loadSuppliers = async () => {
    try {
      ...
      if (response.data) {
        ...
        setSuppliers(suppliers);
        console.log('✅ Suppliers loaded (direct):', suppliers.length);
      }
    } catch (err) {
  ...
  const handleManualAutoTrigger = async () => {
    try {
      ...
      console.log('🚀 Manual auto trigger initiated by:', user.name);
      
      const response = await purchaseRequestApi.triggerAutoGeneration();
      console.log('🚀 Auto trigger response:', response);
      
      if (response.success) {
        const { summary } = response;
        let message = `Auto generation completed!\n`;
        message += `• PRs Created: ${summary.prsCreated}\n`;
        message += `• PRs Auto-Approved: ${summary.prsAutoApproved}\n`;
        message += `• POs Created: ${summary.posCreated}\n`;
        message += `• PRs Skipped: ${summary.prsSkipped}\n`;
        message += `• Low Stock Locations: ${summary.lowStockLocationsFound}`;
        ...
        setError('Failed to trigger auto generation: ' + (response.message || 'Unknown error'));
      }
    } catch (err) {
      console.error('❌ Manual auto trigger error:', err);
      setError('Failed to trigger auto generation: ' + (err.response?.data?.message || err.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateSubmit = async (e) => {
    ...
    console.log('🚀 Submitting PR form:', prForm);
    ...
    try {
      ...
      console.log('📤 Sending PR data:', prData);

      let response;
      if (selectedPR) {
        // Edit mode
        console.log('✏️ Editing PR:', selectedPR.id);
      ...
      if (response.success) {
        ...
        try {
          ...
          console.warn('⚠️ Failed to refresh PR list, but PR was created successfully:', refreshError);
  ...
  const handleCreatePOFromPR = async (prId) => {
    try {
      ...
      if (response.success) {
        ...
        // Handle duplicate PO error specifically
        if (response.message && response.message.includes('already exists')) {
          setError(`Cannot create PO: ${response.message}`);
        } else {
          setError('Failed to create PO from PR: ' + (response.message || 'Unknown error'));
        }
      }
    } catch (err) {
      console.error('Create PO error:', err);
      // Handle duplicate PO error from API response
      if (err.response?.data?.message && err.response.data.message.includes('already exists')) {
        setError(`Cannot create PO: ${err.response.data.message}`);
      } else {
          ...
          {hasOverrideAccess && !canCreatePR && (
            <button
              onClick={() => {
                ...
                if (reason) {
                  console.log(`🚨 Admin Override: ${user.name} - Reason: ${reason}`);
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <div>
            ...
            <select
              ...
              disabled={isWarehouse} // ✅ FIXED: Disable status filter for warehouse staff
            >
            ...
            {isWarehouse && (
              <p className="text-xs text-gray-500 mt-1">Status filter disabled for warehouse staff</p>
            )}
          </div>
                    ...
                    <tr key={pr.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        {pr.status === 'pending' && canConvertToPO ? (
                          ...
                          <div className="flex space-x-2">
                            <button
                              onClick={() => {
                                ...
                                if (reason) {
                                  console.log(`🚨 Admin Override PO Creation: ${user.name} - Reason: ${reason}`);
                          ...
                          <span className="text-blue-600 text-sm">🔄 Converted to PO</span>
                        ) : (
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-screen overflow-y-auto">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4">
                ...
                >
                  ...
                  {items.length > 0 ? items.map(item => (
                    <option key={item.id} value={item.id}>
                      {item.name} ({item.sku}) - RP: {item.reorderPoint || 'N/A'}
                    </option>
                  )) : (
              ...
              <div>
                ...
                {prForm.requestedQuantity && parseInt(prForm.requestedQuantity) <= 0 && (
              ...
              <div>
                ...
                >
                  ...
                  <option value="high">🟡 High - Stock running low</option>
                  <option value="urgent">🔴 Urgent - Out of stock/Critical</option>
                </select>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="low_stock">Low Stock</option>
                  <option value="out_of_stock">Out of Stock</option>
                  <option value="reorder_point">Reorder Point Reached</option>
                </select>
              </div>
