  ...
  try {
    ...
    for (const location of lowStockLocations) {
      try {
        ...
        if (existingPR) {
          console.log(`⏭️ Skipping - PR already exists for ${item.name} at ${warehouse.name}`);
          prsSkipped++;
          continue;
        }
        ...
        if (urgencyLevel === 'urgent' || urgencyLevel === 'high') {
          ...
          try {
            ...
            if (supplier) {
              ...
              console.log(`💰 DEBUG: Full item fetch for unit price:`, {
...
exports.createPOFromPR = async (req, res) => {
  try {
    ...
    const unitPrice = fullItem.unitPrice;
    console.log(`💰 Manual PO: Using actual unit price ${unitPrice} for ${fullItem.name}`);
